...
public class OracleTypeConverter {
    public static Class<?> convertToJavaType(String oracleType) {
        ...
        } else if (oracleType.equalsIgnoreCase("DATE") || oracleType.equalsIgnoreCase("TIMESTAMP")) {
            return String.class; // 또는 java.util.Date, java.time.LocalDateTime 등으로 매핑 가능
        } else if (oracleType.equalsIgnoreCase("BLOB") || oracleType.equalsIgnoreCase("RAW")) {
            return byte[].class;
        }
